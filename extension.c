...
void ELSELIST(Program* p)
{
    if(strsame(p->wds[p->cw], ";")){
        return;
    }
    else if(strsame(p->wds[p->cw], "ELSE")){
        p->cw += 1;
        if(! strsame(p->wds[p->cw], "{")){
...
void ELSEIF(Program* p)
{
    if(! strsame(p->wds[p->cw], "ELSEIF")){
        ERROR("Invalid ELSEIF statement ?\n");
    }
    p->cw += 1;
    POLISHLIST(p);
